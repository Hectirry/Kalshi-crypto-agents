...
class ParamInjector:
    ...
    def __init__(
        self,
        db: Database,
        delta_candidates: tuple[float, ...] = (0.03, 0.05, 0.07, 0.10),
        ev_candidates: tuple[float, ...] = (0.01, 0.02, 0.04, 0.06),
    ) -> None:
        """
    ...
    def _best_threshold(
        ...
        for threshold in candidates:
            ...
            if sample_size == 0:
                ranked.append((threshold, -1.0, float("-inf"), 0))
                continue
            ...
            ranked.append((threshold, win_rate, avg_pnl, sample_size))

        ranked.sort(key=lambda item: (item[1], item[3], item[2], -item[0]), reverse=True)
        best_threshold, best_win_rate, _avg_pnl, best_sample = ranked[0]
